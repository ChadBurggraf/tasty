...
namespace Tasty.Jobs
{
    ...
    internal sealed class JobRunnerProxy : MarshalByRefObject
    {
        ...
        public void StartRunner()
        {
            if (this.isGreen)
            {
                JobRunner.Instance.AllFinished += new EventHandler(this.JobRunnerAllFinished);
        ...
        /// <summary>
        /// Raises the JobRunner.Instance's AllFinished event.
        /// </summary>
        ...
        private void JobRunnerEnqueueScheduledJob(object sender, JobRecordEventArgs e)
        {
            lock (this)
            {
                if (this.EventSink != null)
                {
                    this.EventSink.FireEnqueueScheduledJob(e);
                }
            }
        }
